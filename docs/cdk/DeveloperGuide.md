...
> **Prerequisites:** Tutorials 0-1 require only `pip install modelmesh-lite` and an API key. Tutorials 2-7 assume familiarity with the [CDK Overview](Overview.md) and the [class hierarchy](Overview.md#class-hierarchy). Tutorial 8 is self-contained. Each tutorial references the relevant CDK reference documents inline.
...
> **See also:** [Browser Usage Guide](../guides/BrowserUsage.md) for security considerations, provider-specific notes, and full architecture details.
